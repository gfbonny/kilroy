...
`))
	...
	if got := n.Attrs["llm_model"]; got != "gpt-5.3-codex" {
		t.Fatalf("llm_model: got %q want %q", got, "gpt-5.3-codex")
	}
...
digraph G {
  ...
  start [shape=Mdiamond]
  exit  [shape=Msquare]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="do task a"]
  start -> a -> exit
}
`)
...
digraph G {
  ...
  exit  [shape=Msquare]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex]
  high [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex]
  low  [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex]
  start -> a
  a -> low  [weight=1]
  a -> high [weight=100]
  low -> exit
...
digraph G {
  ...
  exit  [shape=Msquare]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex]
  beta  [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex]
  alpha [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex]
  start -> a
  a -> beta  [weight=5]
  a -> alpha [weight=5]
  beta -> exit
...
digraph G {
  ...
  start [shape=Mdiamond]
  exit  [shape=Msquare]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="do a"]
  start -> a -> exit
}
`)
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
...
digraph G {
  ...
  start [shape=Mdiamond]
  exit  [shape=Msquare]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="Implement: $goal"]
  start -> a -> exit
}
`))
	if err != nil {
...
digraph P {
  ...
  start [shape=Mdiamond]
  par [shape=component]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="branch a"]
  b [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="branch b"]
...
digraph test_pipeline {
    ...
    start       [shape=Mdiamond]
    plan        [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="Plan how to create a hello world script for: $goal"]
    implement   [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="Write the code based on the plan", goal_gate=true]
    review      [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="Review the code for correctness"]
    done        [shape=Msquare]
