...
digraph G {
  ...
  exit  [shape=Msquare]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="Goal is: $goal"]
  start -> a -> exit
}
`))
	if err != nil {
...
digraph G {
  ...
  start [shape=Mdiamond]
  exit  [shape=Msquare]
  verify [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="Lint changed files: git diff --name-only $base_sha | xargs eslint"]
  start -> verify -> exit
}
`))
	if err != nil {
...
digraph G {
  ...
  start [shape=Mdiamond]
  exit  [shape=Msquare]
  a [shape=box, llm_provider=openai, llm_model=gpt-5.3-codex, prompt="Check $base_sha here"]
  start -> a -> exit
}
`))
	if err != nil {
