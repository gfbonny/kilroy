...
digraph G {
  goal="Build the app"
  start [shape=Mdiamond]
  build [shape=box, prompt_file="p.md", llm_provider=openai, llm_model=gpt-5.3-codex]
  exit [shape=Msquare]
  start -> build -> exit
}
`)
...
digraph G {
  start [shape=Mdiamond]
  build [shape=box, prompt_file="nonexistent.txt", llm_provider=openai, llm_model=gpt-5.3-codex]
  exit  [shape=Msquare]
  start -> build -> exit
}
`)
...
digraph G {
  start [shape=Mdiamond]
  build [shape=box, prompt="inline text", prompt_file="extra.md", llm_provider=openai, llm_model=gpt-5.3-codex]
  exit  [shape=Msquare]
  start -> build -> exit
}
`)
