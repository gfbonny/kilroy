...
func TestAdapter_Complete_Usage_MapsReasoningAndCacheTokens(t *testing.T) {
  ...
  "usage": {
	...
	resp, err := a.Complete(ctx, llm.Request{Model: "gpt-5.3-codex", Messages: []llm.Message{llm.User("hi")}})
	if err != nil {
		t.Fatalf("Complete: %v", err)
	}
...
func TestAdapter_Complete_RejectsAudioAndDocumentParts(t *testing.T) {
	...
	_, err := a.Complete(ctx, llm.Request{Model: "gpt-5.3-codex", Messages: []llm.Message{msgAudio}})
	if err == nil {
	...
	_, err = a.Complete(ctx, llm.Request{Model: "gpt-5.3-codex", Messages: []llm.Message{msgDoc}})
	if err == nil {
		t.Fatalf("expected error")
	}
	if !errors.As(err, &ce) {
...
func TestAdapter_Complete_HTTPErrorMapping_IncludesRetryAfter(t *testing.T) {
	...
	_, err := a.Complete(ctx, llm.Request{Model: "gpt-5.3-codex", Messages: []llm.Message{llm.User("hi")}})
	if err == nil {
		t.Fatalf("expected error")
	}
...
func TestAdapter_Stream_YieldsTextDeltasAndFinish(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		write("response.completed", `{"type":"response.completed","response":{"id":"resp_1","model":"gpt-5.3-codex","output":[{"type":"message","content":[{"type":"output_text","text":"Hello"}]}],"usage":{"input_tokens":1,"output_tokens":2,"total_tokens":3}}}`)
	}))
	t.Cleanup(srv.Close)
	...
	stream, err := a.Stream(ctx, llm.Request{Model: "gpt-5.3-codex", Messages: []llm.Message{llm.User("hi")}})
	if err != nil {
...
func TestAdapter_Stream_TranslatesToolCalls(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		write("response.completed", `{"type":"response.completed","response":{"id":"resp_1","model":"gpt-5.3-codex","output":[{"type":"function_call","call_id":"call_1","name":"get_weather","arguments":"{\"n\":1}"}],"usage":{"input_tokens":1,"output_tokens":2,"total_tokens":3}}}`)
	}))
	t.Cleanup(srv.Close)
...
func TestAdapter_Complete_ImageInput_URL_Data_AndFilePath(t *testing.T) {
  ...
  "usage": {"input_tokens": 1, "output_tokens": 1, "total_tokens": 2}
	...
	if _, err := a.Complete(ctx, llm.Request{Model: "gpt-5.3-codex", Messages: []llm.Message{msg}}); err != nil {
		t.Fatalf("Complete: %v", err)
	}
...
func TestAdapter_Stream_ContextDeadline_EmitsRequestTimeoutError(t *testing.T) {
	...
	st, err := a.Stream(ctx, llm.Request{Model: "gpt-5.3-codex", Messages: []llm.Message{llm.User("hi")}})
	if err != nil {
		t.Fatalf("Stream: %v", err)
	}
	defer st.Close()
